...
Specifically, inspired by our work that investigates the abstraction level of training data for code translation~\cite{cmtrans}, which is published in \textcolor{aquablue}{\textbf{EMNLP Findings 2023}}, we identify the execution support for the entire codebase to be the major bottleneck of training environment construction, and explore training settings that avoid code execution or only require execution for a particular module in the codebase.
This simplifies the construction of training environments while preserving training effectiveness~\cite{repost}. This work is published in \textcolor{aquablue}{\textbf{COLM 2025}}.
...
The work has been accepted to \textcolor{aquablue}{\textbf{ICML 2026}}.
...
We first show that the high-level idea of ``training a specialized model to provide guidance to the main model'' can be beneficial in general NLP tasks such as factuality generation~\cite{fence} in an \textcolor{aquablue}{\textbf{ACL 2025}} paper.
...
\begin{figure}[h!]
    ...
    \timelineentry{May 2026 -- Jun 2026}{Submit Proposed Work I (\autoref{chap:critique-training}) to EMNLP 2026}
    \timelineentry{May 2026 -- Sept 2026}{Submit Proposed Work II (\autoref{chap:effective-analysis}) to ICLR 2027}
    \timelineentry{Sept 2026 -- Oct 2026}{Keep Exploring Proposed Work I (\autoref{chap:critique-training})}
    \timelineentry{Oct 2026 -- Jul 2026}{Thesis Writing and Job Search}
    \timelineentry{May 2027}{Thesis Defense}
\end{tikzpicture}
\end{figure}
...
In our work, we propose to train a critique model that is specialized in planning and can provide guidance to the coding agent at inference time, which is a type of direct approach that target the planning bottleneck more explicitly.
Compared to other direct approaches, our approach is more flexible and generalizable for the following reasons:
(1) our critique model can be directly integrated with a fixed coding agent without retraining it. It does not need to be paired with other specialized sub-agents that are each optimized for a particular intermediate step of the task, and
(2) the critique model can be applied at any point during inference and at any desired frequency, thereby enabling flexible control over its use based on user preferences.
...
Specifically, for Proposed Work I (\autoref{part:usage}), we will verify that the characteristics we find indeed affect the overall performance (\autoref{sec:critique-analysis}) and construct SFT data targeting the beneficial or distracting characteristics we find (\autoref{sec:critique-advanced-sft}). 
...
After we submit the paper for Proposed Work II, we will keep exploring the potential to improve the critique model by constructing DPO data (\autoref{sec:critique-dpo}), which approximately takes one month.
...
We aim to finish this stage by the end of July 2026. We will iteratively discover the key agent behaviors and verify their effectiveness. The discovery process is automated with an LLM agent, and the verification process is conducted with manually designed controlled experiments. We approximately need 3-4 days to complete the controlled experiment design, conduct the experiment, run training, and run evaluation for each candidate behavior.
Then we will extend the analysis to multiple training and evaluation tasks and multiple teacher model pairs as proposed in \autoref{plus:sec:extend-exp}. We aim to finish this stage by the end of August 2026. We will then wrap up the experiment results and write a research paper. We aim to submit the paper to ICLR 2027, where the deadline is September 24, 2026.

After submitting the two papers, I will spend the remaining time on thesis writing and job search. I aim to defend the thesis in May 2027.
