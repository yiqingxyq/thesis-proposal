...
% We then show that a critique model that is trained to provide textual critiques for the main model during inference can achieve both principles, for both seq-to-seq NLP tasks (\autoref{chap:fence}) and coding agent tasks (\autoref{chap:critique-training}).
...
% However, as in many machine learning settings, the effectiveness of a coding agent is ultimately bottlenecked by the capacity of its base model. 
However, empirically, we still observe that the performance satuates with respect to data scale after a certain point, and the high-level planning ability remains a major bottleneck.
For instance, prior work~\cite{R2EGym} shows that, when using Qwen2.5Coder-14B as the base model, coding-agent performance already saturates after roughly 800 training trajectories.
...
Finally, we reveal the remaining gaps between the student critique model and the teacher critique model, which motivates our proposed method (\S\ref{sec:critique-proposed}).
...
\start{Experimental Results}
As shown in \autoref{tab:critique:prelim}, we observe that the SFT model can also bring performance gains to the coding agent, improving the resolved rate from 29.4\% to 33.4\%.
...
We present a simple fix to the limit-exceeding problem denoted as ``+ fallback'' in \autoref{tab:critique:prelim}. Specifically, when the coding agent exceeds the limit, we start from the first step again and run inference with the coding agent alone, without any critique models. This improves the non-empty rate from 73.4\% to 94.8\% and improves the resolved rate from 33.4\% to 36.4\%.
