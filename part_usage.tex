...
The key motivation is that the main coding agent is trained to jointly balance multiple abilities, including high-level planning and low-level implementation, and the optinum for jointly optimizing multiple abilities does not necessarily align with the optimum for planning. As a result, training a specialized critique model may potentially lead to stronger planning performance and, in turn, more effective guidance for the coding agent.
...
(1) We can directly use existing coding agent training datasets to construct critique training data, without constructing new training instances or environments, and 
(2) We can train a smaller critique model to assist a larger coding agent (see details in \autoref{sec:critique-sft}), which requires much less computational resources for training.
...
\begin{itemize}
  ...
  \item \textbf{[Potentially harmful behavior \#2] The SFT critique model generates a larger number of instructions than the frontier model}. We observe that the SFT critique model tends to generate longer critiques with a larger number of instructions.
  ...
  \item \textbf{[Potentially harmful behavior \#3] The critique generated by the SFT model does not align well with the coding agent's recent progress}. We observe that there are cases where the SFT critique model generates the same instruction multiple times, even if the coding agent has already completed the first few steps in the instruction. In this way, the coding agent is likely to start over from the first step and stuck in a loop until the step limit is reached.
\end{itemize}

\subsection{Proposed Experiments}
To verify whether these behaviors indeed lead to performance degradation, for each potentially harmful behavior we find, we directly prompt the SFT critique model to avoid this behavior in inference, and compare the performance with the original prompt.

Specifically, to test the effect of ``potentially harmful behavior \#1'', we prompt the SFT critique model with the instruction: ``only generate high-level guidance and do NOT generate concrete actions in the critique'', followed by examples of high-level guidance and concrete actions.

To test the effect of ``potentially harmful behavior \#2'', we prompt the SFT critique model with the instruction: ``do NOT generate more than 3 instructions in the critique''.

To test the effect of ``potentially harmful behavior \#3'', we prompt the SFT critique model with the chain-of-thought process, which first generates the initial set of instructions, checks whether the coding agent has already completed any of the instructions, and then generates the revised set of instructions. We only provide the coding agent with the revised set of instructions.
...
We have already observed that with the revised prompt to test ``potentially harmful behavior \#1'', the overall performance of the coding agent increases from 32\% to 26\% on a subset of 50 SWE-Bench Verified examples, which indicates that it is likely that outputing concrete actions in the critique is harmful to the performance of the coding agent. We will conduct further experiments to test the other two potentially harmful behaviors.
...
(1) the percentage of critiques that avoid the potentially harmful behaviors, and 
