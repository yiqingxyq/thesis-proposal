...
% In particular, we observe that the high-level planning ability remains a major bottleneck. As shown in \autoref{fig:error-analysis}, there is still a substantial gap between the finetuned coding agent and the frontier model in the proportion of examples that exhibit sufficient reasoning.
...
The key motivation is that the main coding agent is trained to jointly balance multiple abilities, including high-level planning and low-level implementation, and theoretically, the optinum for jointly optimizing multiple abilities does not necessarily align with the optimum for planning. 
...
(1) we can directly use existing coding agent training datasets to construct critique training data, without constructing new training instances or environments, 
(2) we can train a smaller critique model to assist a larger coding agent (see details in \autoref{sec:critique-sft}), which requires much less computational resources for training, and 
...
\TBD{Can we claim that ``some API service terms of use do not allow for distillation of reasoning''? I am not sure about this.}
...
As shown in \autoref{tab:critique:prelim}, with Claude-Sonnet-4.6 as the critique model, we significantly improve the performance of CWM-32B~\cite{cwm} on SWE-Bench Verified-mini~\cite{swebench} from 29.4\% to 61.0\%.
Furthermore, we show that a smaller model (Qwen3-8B~\cite{qwen3}) distilled from Claude-Sonnet-4.6's critiques can also bring performance gains from 29.4\% to 35.8\%.
...
Then we show that we can further train a more cost-efficient critique model (e.g., Qwen3-8B) by distilling it from the strong critique model (e.g., Claude-Sonnet-4.6) using vanilla supervised-finetuning (SFT) (\S\ref{sec:critique-sft}).
...
In particular, we focus on the instances where the agent guided by the Claude-Sonnet-4.6 critique model succeeds but the agent guided by the SFT critique model fails.
...
We compare the performance of using the SFT critique model with the original prompt and the revised prompts on the SWE-Bench Verified dataset~\cite{swebench}. If the revised prompts lead to performance gains, we can verify that such behaviors are indeed harmful to the performance of the coding agent.

We have already observed that with the revised prompt to test ``potentially distracting critique \#1'', the overall performance of the coding agent increases from 32\% to 38\% on a subset of 50 SWE-Bench Verified examples, which indicates that it is likely that outputing concrete actions in the critique is harmful to the performance of the coding agent. We will conduct further experiments to test the other two potentially distracting critiques.
...
\subsection{Proposed Experiments}
Similar to the preliminary experiments, we generate a new set of critique data based on 500 issue-solving trajectories, with CWM-32B as the coding agent and Claude-Sonnet-4.6 as the critique model.
We instruct Claude-Sonnet-4.6 with the new prompt and use it to generate critiques every 10 steps.
...
In particular, we apply the SFT critique model to assist CWM-32B on another set of training instances. Whenever we observe that the critique model generates a harmful behavior in the critique, we prompt it to generate additional $n$ critiques (e.g., $n=5$) and use the frontier model (e.g., Claude-Sonnet-4.6) to rank the critiques based on their effectiveness, correctness, and whether they avoid the harmful behaviors.
