...
In \autoref{part:scalability} and \autoref{part:generalizability}, we aim to construct synthetic data to improve coding agent effectiveness. 
However, empirically, we still observe that the performance satuates with respect to data scale after a certain point, and the high-level planning ability remains a major bottleneck.

To address this limitation, in this part, we develop an critique model to provide guidance to the coding agent at inference time. 
...
To demonstrate the utility of critique model, in \autoref{chap:fence}, we show that a critique model can improve the factuality of a generator model by either interacting with it in inference or providing feedback to assist training. We further improve the effectiveness of the critique model by constructing tailored training data.

When it comes to the domain of coding agent, in a \proposedWork (\autoref{chap:critique-training}), we first show that using a frontier model (e.g., Claude) to provide critique during inference substantially improves the correctness of a coding agent. Then we distill a more cost-efficient critique model by constructing synthetic supervised-finetuning training data. To further improve its effectiveness, we propose a method to construct direct preference optimization (DPO) training data, focusing on its gap between the frontier critique model.



\chapter{Improving Generator Factuality by Training an Evaluator Model \textcolor{aquablue}{(\textit{completed})}}
\label{chap:fence}
...
\autoref{chap:critique-training} adapts the high-level idea of auxiliary model training to coding agent, where we first show that the coding agent performs better in inference if it receives critique from an auxiliary model every $k$ steps. Then, we propose a data construction method to train an open-source critique model. 
...
As an industry, it is estimated to be worth more than Â£450m a year to the Welsh economy.
...
\label{chap:critique-training}

\section{Overview}
Synthetic data has proven effective for training coding agents~\cite{swe-smith,R2EGym,hybrid-gym}. 
However, empirically, we observe that for a base model with limited size, the performance satuates with respect to data scale after a certain point.
For instance, prior work~\cite{R2EGym} shows that, when using Qwen2.5Coder-14B as the base model, its performance on the SWE-Bench Verified issue-solving dataset~\cite{swebench} already saturates when trained on roughly 800 issue-solving trajectories.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\columnwidth]{plus_figs/planning_def.pdf}
  \caption{Examples of high-level planning and low-level implementation in coding tasks by our definition. 
  We define ``high-level planning'' as the process of producing intermediate steps about what should be done and in what order, where the intermediate steps do not need to be executable and hence do not have to be syntactically correct.
  We define ``low-level implementation'' as the process of generating concrete executable actions to perform the intermediate steps. The challenge is to generate the action that precisely follows the high-level planning and has the correct syntax (i.e., so that the action can be successfully executed to obtain outputs).}
  \label{fig:plus:planning_def}
\end{figure}

To better understand this limitation, we break down the abilities required for coding agent training into \textbf{high-level planning} (producing intermediate steps about what should be done and in what order) and \textbf{low-level implementation} (generating concrete executable actions to perform the intermediate steps). As shown in \autoref{fig:plus:planning_def}, an example of high-level planning is decomposing the function localization task into discovering the repository structure, localizing relevant files, and locating the relevant functions inside the candidate files.
An example of low-level implementation is calling the ``execute\_bash'' tool to use a ``grep'' command to perform keyword search with the correct syntax.

As pointed out by multiple previous work, even after finetuning, the high-level planning ability of coding agents remains substantially limited.
\citet{swe-empirical-failures} reveals that the most dominant cause for failure in issue-solving (approx. 65\% of cases) is flawed reasoning.
\citet{openhands-rubric-critique} identifies ``insufficient analysis'' as a feature that is highly negatively correlated with the success of pull request merging in real-world software development.
\citet{hybrid-gym} shows that there is still a substantial gap between the finetuned coding agent and the frontier model in the proportion of examples that exhibit adequate reasoning.

In this chapter, following the high-level idea of \autoref{chap:fence}, \textbf{we aim to overcome this bottleneck by training a critique model that specializes in planning and provides guidance to the coding agent during inference.}
The key motivation is that the main coding agent is trained to jointly balance multiple abilities, including high-level planning and low-level implementation, and the optinum for jointly optimizing multiple abilities does not necessarily align with the optimum for planning. 
As a result, training a specialized critique model may potentially lead to stronger planning performance and, in turn, more effective guidance for the coding agent.

Although there could exist approaches to overcome the bottleneck by directly improving the coding agent itself, such as constructing more effective training data (e.g., by constructing training instances more carefully) or using more advanved training paradigms (e.g., reinforcement learning), our approach of critique model training has the following advantages:
(1) we can train a smaller critique model to assist a larger coding agent (see details in \autoref{sec:critique-sft}), which requires much less computational resources for training, 
(2) to train the critique model, we can re-use the same datasets for coding agent training, without constructing new training tasks or environments, 
(3) it provides a computationally cost-efficient way to adjust model behavior during inference, without retraining the main model. 
This is especially compelling for API users, who may not have direct access to the main model.
Furthermore, combining critique model training with the approaches to improve the coding agent itself could potentially lead to additive improvements, because even with the advanced training techniques, the optimum for coding agent training may still be different from the optimum for planning.



In our \textcolor{aquablue}{\textbf{preliminary experiments}}, we show that using a strong critique model to provide free-form textual feedback to the main model during inference can substantially improve the performance.
As shown in \autoref{tab:critique:prelim}, with Claude-Opus-4.6 as the critique model, we significantly improve the performance of CWM-32B~\cite{cwm} on SWE-Bench Verified-mini~\cite{swebench} from 29.4\% to 61.0\%.
Furthermore, we show that a smaller model (Qwen3-8B~\cite{qwen3}) distilled from Claude-Opus-4.6's critiques can also bring performance gains from 29.4\% to 35.8\%.
The results are especially appealing because we demonstrate that it is possible to train a smaller critique model (8B) to assist a larger coding agent (32B), which requires much less computational resources compared to training the coding agent itself.


However, we observe that there is still a large performance gap between the frontier critique model and the SFT-trained critique model.
By manually comparing the trajectories guided by the frontier and SFT critique models, we observe that the SFT critique model generates more distracting critiques that lead to more unnecessary actions (i.e., actions that do not produce useful results for the task), repeating actions, and syntactically incorrect actions.
This also leads to longer trajectory lengths and the coding agent becomes more likely to exceed the budget of 150 agent steps per example before generating a valid code patch. As shown in \autoref{tab:critique:prelim}, with the SFT critique model, the proportion of examples where the coding agent generates a non-empty code patch is even lower than using the coding agent alone (decreasing from 89.0\% to 85.6\%).

To fill the gap, we \textcolor{mediumgreen}{\textbf{propose}} to:
(1) analyze the differences between the critiques generated by the frontier and SFT critique models that could potentially affect the overall performance,
(2) improve the supervised-finetuning (SFT) training data based on the differences we find, and
(3) if the improved SFT data still does not close the gap, we will construct direct preference optimization (DPO) data to further improve the performance of the critique model.


\section{Preliminary Experiments}
In our preliminary experiments, we first show that a strong critique model can bring substantial performance gains to the coding agent in inference (\S\ref{sec:critique-strong}).
Then we show that we can further train a more cost-efficient critique model (e.g., Qwen3-8B) by distilling it from the strong critique model (e.g., Claude-Opus-4.6) using vanilla supervised-finetuning (SFT) (\S\ref{sec:critique-sft}).

\subsection{Inference-time Critique Models can be Effective}
\label{sec:critique-strong}

\start{Experimental Setup}
We experiment with the SWE-Bench Verified dataset~\cite{swebench} with mini-SWE-Agent as the infrastructure~\cite{sweagent}. We use CWM-32B~\cite{cwm} as the coding agent and experiment with three critique models: a frontier model, Claude-Opus-4.6~\cite{claude46}, a cost-efficient open-source model, Qwen3-8B~\cite{qwen3}, and a model finetuned for coding agent tasks, SWE-Smith-7B~\cite{swe-smith}.

Following previous work~\cite{swe-prm}, we adopt the setting where the critique model provides free-form textual critique for the main coding agent every $k$ steps during inference. We use $k=10$ in our experiments. The coding agent can perform at most $150$ steps. After exceeding the limit, the process will be terminated, even if no coding patch is generated.
...
As shown in \autoref{tab:critique:prelim}, we observe that:
(1) The frontier critique model (Claude-Opus-4.6) can bring substantial performance gains to the coding agent, improving the resolved rate from 29.4\% to 61.0\%.
(2) Using an untrained weak critique model such as Qwen3-8B may even reduce the performance of the coding agent, but the performance loss is not significant.
...
\begin{table}[t]
  ...
  Claude-Opus-4.6   & \better \bf 61.0 &  90.0 & 30.28 \\
  \midrule
  Qwen3-8B + SFT & \good 35.8 & 85.6 & 48.01 \\
  % \quad + fallback & \good 37.4 & 96.2 & \textemdash \\
  \bottomrule
  \end{tabular}%
  }
  \caption{Results on equipping CWM-32B with different critique models on SWE-Bench Verified. 
  % ``fallback'' refers to the setting where if the coding agent exceeds the limit, we run inference on this example again without the critique model.
  }
  \label{tab:critique:prelim}
\end{table}
...
%   \toprule
%   \multirow{2}{*}{\bf Crtique Model} &
%   \multicolumn{1}{c}{\bf SWE-Bench Verified} \\
%   % \cmidrule(lr){2-2}
%   & Resolved \% \\
%   \midrule
%   \sdlrow 
%   \multicolumn{2}{c}{\textit{Coding Agent: CWM-32B}} \\
%   \midrule
%   \textemdash & 29.4 \\
%   \midrule
%   Qwen3-8B & \bad 24.6 \\
%   SWE-Smith-7B & \bad 21.2 \\
%   Claude-Opus-4.6   & \better \bf 61.0 \\
%   \midrule
%   Qwen3-8B + SFT & \good 35.8 \\
%   % \quad + fallback & \good 37.4 & 96.2 & \textemdash \\
%   \bottomrule
%   \end{tabular}%
%   }
...
\label{sec:critique-sft}
\start{Experimental Setup}
To reduce inference cost, we distill Qwen3-8B from Claude-Opus-4.6 using vanilla supervised-finetuning (SFT).

We build training data from 500 trajectories with CWM-32B as the coding agent and Claude-Opus-4.6 as the critique model. For each piece of critique, we use the partial trajectory before it as the prefix and optimize the probability of the critique.

We train the model on 8 L-40 GPUs, with a batch size of 8, and a learning rate of 5e-6. We train the model for 3 epochs.

\start{Experimental Results}
As shown in \autoref{tab:critique:prelim}, we observe that the SFT model can also bring performance gains to the coding agent, improving the resolved rate from 29.4\% to 35.8\%.
However, the performance gap with using the frontier critique model is still large, indicating that there is still a large room for improvement. In our proposed methods, we will discuss the root causes of the performance gap and propose methods to close the gap.




\section{Proposed Method 1: Analysis of Distracting Critiques}
\label{sec:critique-analysis}
\subsection{Proposed Method}
We manually compare the critiques generated by the frontier and SFT critique models to identify the differences that could potentially affect the overall performance.
In particular, we focus on the instances where the agent guided by the Claude-Opus-4.6 critique model succeeds but the agent guided by the SFT critique model fails.

Below is a list of differences that we already observed, and why we assume they may affect the overall performance. 
Note that the list is not complete and will be updated as we conduct further analysis.
\begin{itemize}
  \item \textbf{[Potentially distracting critique \#1] The SFT critique model generates more concrete actions than the frontier model}. We observe that there are a number of cases where the frontier model generates the general plan (e.g., ``you can use keyword search to locate relevant files''), and the SFT critique model generates the concrete actions to execute the plan (e.g., the exact ``grep'' command to perform keyword search).
  However, since the SFT critique model is not directly optimized for generating actions and it has a smaller size than the main coding agent, it is likely that compared to the coding agent, the critique model has weaker capability in generating actions that both follow the high-level planning and have the correct syntax.
  As a result, it is not reasonable for the coding agent to follow the concrete actions generated by the SFT critique model.
  \item \textbf{[Potentially distracting critique \#2] The SFT critique model generates a larger number of instructions than the frontier model}. We observe that the SFT critique model tends to generate longer critiques with a larger number of instructions.
  We assume that it could potentially affect the performance because we observe that the instruction-following ability of the coding agent is limited. When we provide it with too many instructions, where some of the instructions are more essential than others, the coding agent does not always start with the most essential instruction.
  \item \textbf{[Potentially distracting critique \#3] The critique generated by the SFT model does not align well with the coding agent's recent progress}. We observe that there are cases where the SFT critique model generates the same instruction multiple times, even if the coding agent has already completed the first few steps in the instruction. In this way, the coding agent is likely to start over from the first step and stuck in a loop until the step limit is reached.
\end{itemize}

\subsection{Proposed Experiments}
To verify whether these behaviors indeed lead to performance degradation, for each potentially distracting critique we find, we directly prompt the SFT critique model to avoid this behavior in inference, and compare the performance with the original prompt.

Specifically, to test the effect of ``potentially distracting critique \#1'', we prompt the SFT critique model with the instruction: ``only generate high-level guidance and do NOT generate concrete actions in the critique'', followed by examples of high-level guidance and concrete actions.

To test the effect of ``potentially distracting critique \#2'', we prompt the SFT critique model with the instruction: ``do NOT generate more than 3 instructions in the critique''.

To test the effect of ``potentially distracting critique \#3'', we prompt the SFT critique model with the chain-of-thought process, which first generates the initial set of instructions, checks whether the coding agent has already completed any of the instructions, and then generates the revised set of instructions. We only provide the coding agent with the revised set of instructions.

We compare the performance of using the SFT critique model with the original prompt and the revised prompts on the SWE-Bench Verified dataset~\cite{swebench}. 
We also compute (1) the percentage of critiques that contain concrete actions, (2) the average number of instructions in the critiques, and (3) the percentage of repeating critiques to verify whether the revised prompts reduce those potentially distracting behaviors.
If the revised prompts lead to performance gains and the potentially distracting behaviors are reduced, we can verify that such behaviors are indeed harmful to the performance of the coding agent.

We have already observed that with the revised prompt to test ``potentially distracting critique \#1'', the overall performance of the coding agent increases from 32\% to 38\% on a subset of 50 SWE-Bench Verified examples and the proportion of critiques that contain concrete actions is reduced, which indicates that it is likely that outputing concrete actions in the critique is harmful to the performance of the coding agent. We will conduct further experiments to test the other two potentially distracting critiques.



\section{Proposed Method 2: Improving SFT Data to Distill Critique Models}
\label{sec:critique-advanced-sft}

\subsection{Proposed Method}
After identifying the behaviors that are potentially harmful to the performance of the coding agent (\autoref{sec:critique-analysis}), we propose to improve the SFT data by avoiding these behaviors in the training data.
In particular, when generating SFT data, we prompt the frontier critique model to avoid such behaviors, using the same instructions introduced in \autoref{sec:critique-analysis}.

In case that the frontier critique model fails to follow the prompt, we perform a data filtering step and only include critiques that do not contain any concrete actions, do not contain more than 3 instructions, etc., in the SFT data for distillation.


\subsection{Proposed Experiments}
Similar to the preliminary experiments, we generate a new set of critique data based on 500 issue-solving trajectories, with CWM-32B as the coding agent and Claude-Opus-4.6 as the critique model.
We instruct Claude-Opus-4.6 with the new prompt and use it to generate critiques every 10 steps.

We use an LLM (e.g., GPT-5-mini~\cite{gpt5}) to check whether the critiques follow all the requirements in the new prompt, and only include critiques that pass the check in the SFT data for distillation.

We distill a new critique model with the new SFT data, with Qwen3-8B as the base model and the exact same training settings as the preliminary experiments. 
Then we apply the new critique model to assist CWM-32B on the SWE-Bench Verified dataset~\cite{swebench}.
We compare to the original SFT critique model on:
(1) the percentage of critiques that avoid the potentially distracting critiques, and 
(2) the final performance of the coding agent (i.e., the resolved rate).



\section{Proposed Method 3: Constructing DPO Data to Distill Critique Models}
\label{sec:critique-dpo}
\subsection{Proposed Method}
Note that even with the improved SFT data, with no harmful behaviors in the training critiques, it is still possible that the distillation is not effective enough and the student critique model still has harmful behaviors in some of the critiques.

To further improve the performance of the critique model, we propose to add a direct preference optimization (DPO) stage after SFT.

In particular, we apply the SFT critique model to assist CWM-32B on another set of training instances. Whenever we observe that the critique model generates a harmful behavior in the critique, we prompt it to generate additional $n$ critiques (e.g., $n=5$) and use the frontier model (e.g., Claude-Opus-4.6) to rank the critiques based on their effectiveness, correctness, and whether they avoid the harmful behaviors.
We use the critique with the highest score as the preferred response and the critiques with the lowest scores as the rejected responses, and train the critique model with DPO.



\subsection{Proposed Experiments}
We generate 300 pairs of preferred and rejected critiques, and train the SFT critique model with DPO.
We conduct hyper-parameter search over the learning rate, batch sie, and number of training epochs.

Similar to \autoref{sec:critique-advanced-sft}, we apply the DPO-trained critique model to assist CWM-32B on the SWE-Bench Verified dataset~\cite{swebench}.
Then we compare the proportion of harmful behaviors and the final resolved rate with the SFT critique model.
